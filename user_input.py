...
def user_features(preprocessed_data):
    ...
    if flag <= (0.5 * (len(QU_vectorizer.get_feature_names()))):
        is_in_dataset = False

    print(QU_features.shape)
    return QU_features, is_in_dataset


def process_user_input(user_string):
    preprocessed_string = preprocessing_test_data(user_string)
    features, bool_value = user_features(preprocessed_string)
    ...
    if bool_value == False:
      content = 'لقد أرسل عميل هذه الرسالة : \n ' +  '"' +  user_string + '"' + '\n  برجاء التواصل معه'
      mail = smtplib.SMTP('smtp.gmail.com', 587) #or 465 or 587
      mail.ehlo()     #identify urself to the server
      mail.starttls() #anything follows is encrypted
      mail.login('CRAchatbot13619@gmail.com', '123456CRA')
      SUBJECT = 'CRA chatbot'
      msg = 'Subject: {}\n\n{}'.format(SUBJECT, content)
      mail.sendmail('CRAchatbot13619@gmail.com', 'esraa.hisham@cis.asu.edu.eg', msg.encode())
      mail.close()
      return "عذرا لم افهم سؤالك، سيتم توجيهك الي احد من خدمة العملاء"
